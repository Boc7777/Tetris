...
Block_S::Block_S(float _cell_size, Color _color, RenderWindow* _window, vector<TetrisBlock*> _blocks_tab) {
...
void Block_S::createShape() {
	mini_cells_tab[0].setPosition(Vector2f(cell_size * 6, cell_size * -2));
	mini_cells_tab[1].setPosition(Vector2f(cell_size * 5, cell_size * -2));
	mini_cells_tab[2].setPosition(Vector2f(cell_size * 5, cell_size * -1));
	mini_cells_tab[3].setPosition(Vector2f(cell_size * 4, cell_size * -1));

	setBlockPosition(Vector2f(mini_cells_tab[3].getPosition().x, mini_cells_tab[3].getPosition().y-cell_size));
}
...
RenderWindow* Block_S::getWindow() {
	return window;
}

vector<TetrisBlock*> Block_S::getTabBloks() {
	return blocks_tab;
}

vector<Cell>& Block_S::getTabCells() {
	return mini_cells_tab;
}

float Block_S::getCellSize() {
	return cell_size;
}

void Block_S::setBlockUnderControl(bool _bool) {
	block_under_control = _bool;
}

Color Block_S::getColor() {
	return color;
}

Vector2f& Block_S::GetBlockPosition() {
	return blockPosition;
}

void Block_S::setBlockPosition(Vector2f position) {
