...
Block_Z::Block_Z(float _cell_size, Color _color, RenderWindow* _window, vector<TetrisBlock*> _blocks_tab) {
...
RenderWindow* Block_Z::getWindow() {
	return window;
}

vector<TetrisBlock*> Block_Z::getTabBloks() {
	return blocks_tab;
}

vector<Cell>& Block_Z::getTabCells() {
	return mini_cells_tab;
}

float Block_Z::getCellSize() {
	return cell_size;
}

void Block_Z::setBlockUnderControl(bool _bool) {
	block_under_control = _bool;
}

Color Block_Z::getColor() {
	return color;
}

Vector2f& Block_Z::GetBlockPosition() {
	return blockPosition;
}

void Block_Z::setBlockPosition(Vector2f position) {
